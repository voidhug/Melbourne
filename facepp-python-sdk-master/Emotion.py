...
def like_percent(num):
    return -200/(1 + numpy.e**(-num)) + 200

def proc_diff(data_I, data_L, wL=wL, wT=wT, like=True):
    ...
    if like:
        return _v,like_percent(_v)
    else:
        return _v

if __name__ == '__main__':
    answer_I = process_data(square, example_data_I)
    answer_T = process_data(square, example_data_L)
    print answer_I,'\n\n\n\n'
    print answer_T
    _v = diff(
        [drag_L(answer_I), process_deforation(example_data_I)],
        [drag_L(answer_T), process_deforation(example_data_L)]
        )
    print _v, like_percent(_v)
